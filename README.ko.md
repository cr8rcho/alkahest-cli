...
| `list_tasks` | **내 개인 task** 읽기 — 나만 보는 사적 체크리스트. 기본 `open`, `status: all` 이면 완료 포함, `project` 로 프로젝트 태그 필터 (토큰 필요) |
| `add_task` | 사용자의 **개인** task 추가 — 사적이고 홈 Tasks 밴드 + 활동 피드에 (에이전트로 표시돼) 뜬다. **프로젝트·publish 불요** — `project` 생략 시 개인 Inbox, `project`(slug) 로 태그, `tags` 로 자유 라벨. 나중에 이슈로 승격 가능; 팀 공유 업무면 `add_issue`. `dedup_key` 로 재스캔 멱등성 (토큰 필요) |
