...
const preset = program
  .command("preset")
  .description("installable presets — packaged conventions your agent follows: account skills + note maps, plus a repo scaffold / sync script / CLAUDE.md snippet when the preset has them (as-built docs, llm-wiki)");
preset
  .command("list")
  .description("list the presets in the registry (the same list the web gallery and the MCP `presets` tool read)")
  .action(() => presetsList());
preset
  .command("install <id>")
  .description("install a preset: account skills (upsert by name, existing kept) + note maps, and the repo half when it has one (scaffold, reference script, CLAUDE.md snippet — opt-in append)")
  ...
  .option("--path <dir>", "repo root for the preset's files, if it has any", ".")
  .option("--slug <slug>", "project slug for the note maps (defaults to the saved slug for --path; maps are skipped without one)")
  .option("--api <url>", "API base URL (or env ALKAHEST_API_URL)")
...
const docs = program
  .command("docs")
  .description("alias — `docs init` is `preset install` (defaults to the as-built docs preset)");
docs
  .command("init")
  .description("alias of `alkahest preset install <id>` (default: as-built)")
  .option("--preset <id>", "preset id (see `alkahest preset list`)", "as-built")
  .option("--force", "overwrite existing same-name skills with the preset bodies (default: keep yours)", false)
  .option("--claude-md", "append the CLAUDE.md snippet without prompting (non-interactive runs print it instead)", false)
