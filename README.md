...
alkahest preset update [id]     # bring installed preset skills / reference script / CLAUDE.md snippet up to the current version: unedited copies are replaced, edited ones merged (your edits kept; where an edit and a preset change hit the same lines the preset's win and the replaced lines are listed). --dry-run, --json
...
| `add_task` | add a **personal** task to the user's list — private, shows in their home Tasks band + activity feed (chipped as the agent). **No project or publish required** — omit `project` for a personal Inbox task, or tag one with `project` (slug); add free `tags`. Promotable to an issue later; for shared/team work use `add_issue`. Put the context — why it matters, what "done" looks like, and a `[label](url)` link to the evidence — in `body`, which renders as markdown; there is no separate link field. `dedup_key` keeps re-scans idempotent (needs a token) |
| `promote_task` | turn a personal task into a full issue — the **same verb as the web's "Promote to issue"**: issue from the task's title/body, lineage kept (the task leaves the open list), the task's thread moves onto the issue, one Promoted row in the feed. Use this instead of `add_issue` + `complete_task` when a task grows teeth. Needs a project-filed task + editor access; `map` places it on an issue map (default: pool) |
